...
class Detector:
    def __init__(self, source, veh_model="yolo11n.pt", plate_model="models/license_plate.pt",
        ...
        if not os.path.exists(plate_model):
            raise FileNotFoundError(f"License plate model not found at {plate_model}.")
    ...
    def _open_writer(self, path, fps, width, height):
        """Try multiple codec options for maximum compatibility"""
        codecs = [
            ('mp4v', '.mp4'),  # MP4V
            ('XVID', '.avi'),  # XVID AVI
        ...
        for codec_str, ext in codecs:
            try:
                ...
                if writer.isOpened():
                    print(f"[INFO] VideoWriter opened with {codec_str} codec: {test_path}")
                    return writer, test_path
                else:
                    writer.release()
            except Exception as e:
                print(f"[DEBUG] Failed codec {codec_str}: {e}")
                continue
    ...
    def run(self):
        ...
        print(f"[INFO] Input: {orig_w}x{orig_h} @ {fps}fps, {total_frames} frames")
        ...
        try:
            ...
            if not writer:
                print("[WARNING] Could not create video writer. Proceeding without video output.")
            ...
            while True:
                ...
                if frame_id % 30 == 0:
                    ...
                    print(f"[PROGRESS] Frame {frame_id}/{total_frames} ({pct:.1f}%) - {fps_proc:.1f} fps")
                ...
                for p in plates:
                    ...
                    if px2 > px1 and py2 > py1:
                        ...
                        # Attach to nearest vehicle by IoU
                        best_iou = 0.0
                        best_vi = None
                        ...
                        if best_vi is not None and best_iou > 0.02:
                ...
                for v in vehicles:
                    ...
                    if v.get('plate'):
                        label += f" | {v['plate']}"
                    cv2.putText(ann, label, (x1, max(15, y1-6)), 
                ...
                for p in plates:
                    ...
                    if p.get('text'):
                        cv2.putText(ann, p['text'], (x1, max(15, y1-6)), 
                                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
                
                # Overlay counts
                y0 = 20
                for i, (k, v) in enumerate(self.counts.items()):
                    cv2.putText(ann, f"{k}: {v}", (10, y0 + i*22), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
            ...
            print("\n[INFO] Interrupted by user. Saving outputs...")
        except Exception as e:
            ...
            if writer:
                writer.release()
                print(f"[INFO] Released video writer")
            ...
            with open(summary_path, "w") as f:
                if self.counts:
                    ...
                    f.write("No vehicles counted.\n")
            print(f"[INFO] Saved counts: {summary_path}")
            
            if out_used and os.path.exists(out_used):
                print(f"[INFO] Annotated video: {out_used}")
            else:
                print("[WARNING] No video output was created")
            
            elapsed = time.time() - start_ts
            print(f"[INFO] Processed {processed_frames} frames in {elapsed:.1f}s ({processed_frames/elapsed:.1f} fps)")
...
if __name__ == "__main__":
    args = parse_args()
    det = Detector(source=args.source, veh_model=args.veh_model, 
                   plate_model=args.plate_model, out_dir=args.out, 
                   skip=args.skip, resize_width=args.resize_width,
